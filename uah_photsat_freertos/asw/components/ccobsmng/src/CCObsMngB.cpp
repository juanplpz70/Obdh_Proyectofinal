...
VNextTimeOut+= Pr_Time(0,75000); // Add X sec + Y microsec 
time=VNextTimeOut; 
   //Program absolute timer 
   AttCtrlTimer.InformAt( time ); 
}
	...
	do
	{
		...
		switch(edroomCurrentTrans.localId)
		{
			...
			case (Init):
				...
				break;
			//To Choice Point DoAttitutdeCtrl
			case (DoAttitutdeCtrl):
				...
				if( GReadyToObservation() )
				{
					...
					//Branch taken is DoAttitutdeCtrl_ReadyToObservation
					edroomCurrentTrans.localId =
						DoAttitutdeCtrl_ReadyToObservation;
				...
				else
				{
					...
					//Branch taken is DoAttitutdeCtrl_ProgAttitudeCtrl
					edroomCurrentTrans.localId =
						DoAttitutdeCtrl_ProgAttitudeCtrl;
			...
			case (TakeImage):
				...
				//Execute Action 
				FTakeImage();
				//Evaluate Branch EndObservation
				if( GLastImage() )
				{
					//Execute Actions 
					FEndObservation();
					...
					//Branch taken is TakeImage_EndObservation
					edroomCurrentTrans.localId =
						TakeImage_EndObservation;
		...
		switch(Msg->signal)
		{
			...
			case (EDROOMSignalTimeout): 
				 ...
				 if (*Msg->GetPInterface() == AttCtrlTimer)
				{
					...
					//Next transition is  DoAttitutdeCtrl
					edroomCurrentTrans.localId = DoAttitutdeCtrl;
					edroomCurrentTrans.distanceToContext = 0 ;
					edroomValidMsg=true;
				 }
